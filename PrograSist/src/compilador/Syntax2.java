...
//		check+=tablatokens.getLast().token+" "+tablatokens.getLast().nombre+" "+tablatokens.getLast().tipo+" "+tablatokens.getLast().valor+"\n";}
...
//		JOptionPane.showMessageDialog(null,check);
	...
	public static boolean Constante(){
		if(tiraTokens.startsWith("80")){
			for(int i=0;;i++)
				if(tiraTokens.startsWith(tablatokens.get(i).token)){
					//JOptionPane.showMessageDialog(null,"Encontrado: "+tablatokens.get(i).valor);
					usadas.add(tablatokens.get(i)); break;}
						...
						if(tiraTokens.startsWith(""+(ScanLex.reservadas.length))){	//';'
							...
							if(checarTipo()){
								if(usadas.get(0).tipo.equals("CARACTER")){
									...
									contin=false; ScanLex.error+="Asignación inadecuada en <Asigna>\n";
									return false;}
								JOptionPane.showMessageDialog(null,usadas.get(0).nombre+":="+usadas.get(0).valor);
								return true;}
